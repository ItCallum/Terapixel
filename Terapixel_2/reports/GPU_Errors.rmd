...
So we now know that something caused an increase in upload times between 07:41:00 and 07:44:00. The question is can we work out why this is happening. If we can work why then measures could be put in place to try and reduce these problems in the future.#

Lets try and find what these GPUs have in common. I'm looking at all GPUS that at some point have a upload time of 50 second. It turns out we have 776 GPUS which where used in these 977 task.

My first idea is that it might be because of because of these GPUS being used first to upload job lvl 8 tasks. As these are done first there might be a possibility that switching from task 8 to task 12 causes this issue. This isnt the case as only 109 task where tied job 8  
```{r could_it_be_run}

all_high_uploads <- times %>% na.omit(times) %>% filter(jobId == "1024-lvl12-7e026be3-5fd0-48ee-b7d1-abd61f747705", eventName == "Uploading", diff > 15)
