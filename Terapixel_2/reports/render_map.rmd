...
To di this i created a bit of python code that would allow me to apply a K-means clustering algorithm to an image so that I could perform Color Quantization. Color Quantization is the process of reducing number of colors in an image, I wanted to do this to make classifying what is in a tile easier. 
...
task_join_col %>%  group_by(dominate)  %>%  summarize(min = min(diff) , qt1 = quantile(diff, 1/4), median =  median(diff), qt3 = quantile(diff, 3/4), max =max(diff))
...
#plots(
#  ggplot(task_join_col, aes(x, y, fill= extra_shadows)) +  geom_tile() + scale_fill_gradient(low="grey", high="blue") + ggtitle("Roof_Shadows percenatge each tile"),
 # ggplot(task_join_col, aes(x, y, fill= diff)) +  geom_tile()  + scale_fill_gradient(low="grey", high="blue") + ggtitle("Render time of each tile in Job 8"),
 # n_columns = 2
#)
